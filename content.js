(function () {
  ...
  const DIRECT_AD_TOKEN_RE = /(^|[^a-z0-9])(ad|ads|ad-slot|ad-container|advert|advertisement|sponsor(?:ed|ship)?|promo(?:tion)?|banner)([^a-z0-9]|$)/i;
    ...
    panel.innerHTML = `
      <div class="ad-cleaner-panel-shell">
        <header class="ad-cleaner-panel-header">
          <div>
            <div class="ad-cleaner-panel-kicker">Recovery</div>
            <div class="ad-cleaner-panel-title">误杀恢复面板</div>
          </div>
          <button type="button" data-action="close">关闭</button>
        </header>
        <div class="ad-cleaner-panel-toolbar">
          <button type="button" data-action="restore-all">全部恢复</button>
      ...
      .map((entry) => `
        <div class="ad-cleaner-panel-row">
          <div class="ad-cleaner-panel-meta">
            ...
            <div class="ad-cleaner-panel-row-sub">原因：${escapeHtml(entry.reason)}</div>
          </div>
          <button type="button" data-action="restore-one" data-id="${escapeAttr(entry.id)}">恢复</button>
        </div>
      `)
      .join("");
  }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === "GET_STATUS") {
      ...
      sendResponse({
        ...
        pageProfile: tryBlockBasicPage ? "TryBlock 基础测试页" : tryBlockIntermediatePage ? "TryBlock 中级测试页" : tryBlockAdvancedPage ? "TryBlock 高级测试页" : lowFalsePositiveMode ? "低误杀模式" : "标准模式",
  ...
  function startElementPicker() {
    if (pickerActive) {
      setPickerMessage("点选屏蔽已开启，点击一个广告块即可。");
      return;
    }
    ...
    setPickerMessage("点选屏蔽已开启，点击广告块保存选择器，Esc 取消。");
  ...
  function commitPickerSelection(element) {
    ...
    if (!selector) {
      stopElementPicker("没能为这个元素生成稳定选择器。");
      return;
    }

    saveManualSelector(selector)
      .then(() => {
        setPickerMessage(`已添加屏蔽规则：${selector}`);
        scanDocument();
        renderRecoveryPanel();
        stopElementPicker("已保存到当前站点的屏蔽规则。");
      })
      .catch((error) => {
        stopElementPicker(`保存失败：${String(error?.message || error)}`);
      });
  }
  ...
  function buildPickerOverlay() {
    ...
    panel.innerHTML = `
      <div class="ad-cleaner-picker-shell">
        <div class="ad-cleaner-picker-title">点选屏蔽模式</div>
        <div class="ad-cleaner-picker-copy">点击页面里的广告块，自动生成当前站点的隐藏规则。按 Esc 退出。</div>
        <div class="ad-cleaner-picker-message" data-role="message"></div>
        <button type="button" class="ad-cleaner-picker-cancel" data-action="cancel">取消</button>
      </div>
    `;
