...
async function getRuleMatches(tabId) {
  ...
  if (!Number.isInteger(numericTabId) || numericTabId < 0) {
    return { ok: false, error: "当前标签页不可诊断" };
  }

  if (typeof chrome.declarativeNetRequest.getMatchedRules !== "function") {
    return { ok: false, error: "当前浏览器不支持规则命中诊断" };
  }
...
async function updateBadgeForTab(tabId, result) {
  ...
  const title = result.source === "page"
    ? (total ? `Ad Cleaner · 页面已隐藏 ${total} 个广告元素` : "Ad Cleaner · 当前页面暂无广告元素")
    : (total ? `Ad Cleaner · 已拦截 ${total} 个链接` : "Ad Cleaner · 当前页面暂无网络命中");
...
async function compileSubscriptions(subscriptions, maxRuleCount = MAX_COMPILED_SUBSCRIPTION_RULES, previousCache = {}, previousHideRules = {}) {
  ...
  for (const subscription of subscriptions) {
    ...
    if (!loadResult.text) {
      ...
      if (cachedRules.length) {
        mergeHideRules(hideRules, previousHideRules);
      }
      subscriptionStats[subscription.id] = {
        id: subscription.id,
        name: subscription.name,
        ...
        message: cachedRules.length
          ? `${loadResult.errorMessage || "加载失败"}，继续使用上次成功版本`
          : loadResult.errorMessage || "加载失败",
    ...
    subscriptionStats[subscription.id] = {
      ...
      message: compiled.blockRules.length || Object.keys(compiled.hideRules).length ? "已生效" : "未识别到可用规则",
  ...
  for (const stats of Object.values(subscriptionStats)) {
    ...
    if (stats.status === "active" && stats.addedRuleCount < stats.ruleCount) {
      stats.message = `已生效，实际加入 ${stats.addedRuleCount} 条`;
    }
  }
...
async function loadSubscriptionText(subscription) {
  ...
  try {
    ...
    if (!sourceUrl) {
      return {
        text: "",
        errorMessage: "订阅地址无效，请填写完整的规则 URL"
      };
    }
    ...
    if (looksLikeHtmlPage(text)) {
      return {
        text: "",
        errorMessage: "返回的是网页 HTML，不像过滤规则文本"
      };
    }
...
function parseSubscriptionMetadata(text) {
  ...
  if (!metadata.sourceSummary) {
    ...
    if (sourceLine) {
      metadata.sourceSummary = sourceLine.replace(/^[!#]+\s*/, "").replace(/^(source|规则源)\s*[:：]\s*/i, "").trim();
    }
  }
